...
def get_new_mail(mailbox_admin, request, queryset):
    for mailbox in queryset.all():
        ...
        if len(got_mail) > 0:
            messages.success(
                request,
                _('Got {} new letters for mailbox "{}"').format(
                    str(len(got_mail)), mailbox.name
                ),
            )
        else:
            messages.info(
                request, _('No new mail for mailbox "{}"').format(mailbox.name)
            )
...
def send_queued_mail(outbox_admin, request, queryset):
    ...
    for outbox in queryset.all():
        ...
        logger.info(
            "Acquiring lock for sending queued emails at %s.lock" % default_lockfile
        )
        try:
...
class IncomingEmailAdmin(admin.ModelAdmin):
    ...
    def mailbox_link(self, msg):
        return mark_safe(
            '<a href="'
            + reverse("admin:django_mail_admin_mailbox_change", args=[msg.mailbox.pk])
            + '">'
            + msg.mailbox.name
            + "</a>"
        )
    ...
    def reply_link(self, msg):
        if msg.in_reply_to:
            return mark_safe(
                '<a href="'
                + reverse(
                    "admin:django_mail_admin_outgoingemail_change",
                    args=[msg.in_reply_to.pk],
                )
                + '">'
                + msg.in_reply_to.subject
                + "</a>"
            )
        else:
            return ""
    ...
    def envelope_headers(self, msg):
        ...
        return "\n".join([("%s: %s" % (h, v)) for h, v in email.items()])
...
if admin_row_actions:
    ...
    def get_row_actions(self, obj):
        row_actions = [
            {
                "label": _("View emails"),
                "url": reverse("admin:django_mail_admin_incomingemail_changelist")
                + "?mailbox__name="
                + obj.name,
