...
def get_available_backends():
    ...
    if backend:
        warnings.warn(
            'Please use the new DJANGO_MAIL_ADMIN["BACKENDS"] settings',
            DeprecationWarning,
        )
    ...
    backends["default"] = getattr(
        settings, "EMAIL_BACKEND", "django_mail_admin.backends.CustomEmailBackend"
    )
...
def get_altered_message_header():
    return get_config().get(
        "ALTERED_MESSAGE_HEADER", "X-Django-Mail-Admin-Altered-Message"
    )


def get_attachment_interpolation_header():
    return get_config().get(
        "ATTACHMENT_INTERPOLATION_HEADER", "X-Django-Mail-Admin-Interpolate-Attachment"
    )
...
def get_nylas_api_key():
    """Get Nylas API key from Django settings"""
    return getattr(settings, "NYLAS_API_KEY", None)
